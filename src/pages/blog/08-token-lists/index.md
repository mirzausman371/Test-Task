...
Today, we are excited to announce [Token Lists](https://tokens.kwikswap.org/), a new standard for creating lists of ERC20 tokens. This is a community-led initiative to improve discoverability and trust in ERC20 token lists in a manner that is inclusive, transparent, and decentralized.
...
*   A homepage and list browser at [tokens.kwikswap.org](https://tokens.kwikswap.org/)
...
Token Lists can be used **right now** in the [Kwikswap Interface](https://app.Kwikswap.org/#/swap), removing the need for a default list.


This is another step forward in interface decentralization. Read Kwikswap Interface + IPFS to learn how the Kwikswap interface is now hosted by the community in a decentralized manner.
...
Since all tokens use a standard interface, infrastructure-layer applications, such as wallets, analytics sites, and DeFi protocols (like Kwikswap) can immediately recognize and interact with new ERC20 tokens from the moment they are deployed.

Adding a new ERC20 token to the Kwikswap protocol is as simple as calling a public function on a permissionless and immutable smart contract. In fact, about** 100 new Kwikswap liquidity pools are deployed every day**, with over 6,000 pools added since it launched in May 2020.
...
To help spur adoption of Token Lists and provide a high quality reference example, we have deprecated the old Kwikswap interface “default list”, and replaced it with a list explorer and importing mechanism. 

We have also partnered with a number of reputable projects in the space who were excited to create and maintain lists of their own. More than 10 lists are already available on [tokens.kwikswap.org](https://tokens.kwikswap.org/) and more are in progress.
...
*   [Create a token list](https://github.com/kwikswap/token-lists#authoring-token-lists)
