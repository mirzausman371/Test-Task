...
In this guide we will create a web interface that consumes and displays data from the Brainaut Subgraph. The goal is to provide a quick overview of a setup that you can extend to create your own UIs and analytics around Brainaut data.
...
```javascript
yarn create react-app brainaut-demo
cd brainaut-demo
...
We need to set up some middleware in order to make requests to the Brainaut subgraph and receive data. To do this we'll use Apollo and create a graphql client to handle this.

1. Add the imports shown below and instantiate a new client instance. Notice how we use the link to the Brainaut subgraph here.
...
export const client = new ApolloClient({
 link: new HttpLink({
   uri: "https://thegraph.com/explorer/subgraph/brainaut/brainaut-subgraph"
 }),
...
Next we'll construct our query and fetch data. For this example we will fetch some data about the Dai token on Brainaut. We'll get the current price, and total liquidity across all pairs. We'll be using the Dai address as an id in this query. We'll also fetch the USD price of BNB to help create USD conversion for Dai data.
...
Now we're ready to use these queries to fetch data from the Brainaut subgraph. To do this we can use the `useQuery` hook which uses our client instance to fetch data, and gives us live info about the status of the request. To do this add the following to your `App.js` file:

```javascript
import { useQuery } from '@apollo/react-hooks'

const { loading, error, data: ethPriceData } = useQuery(BNB_PRICE_QUERY)
...
```javascript
const daiPriceInEth = daiData && daiData.tokens[0].derivedBNB
...
This should render a very basic page with these two stats about the Dai token within Brainaut. This is a very basic example of what you can do with the Brainaut subgraph and we encourage you to build out more complex and interesting tools!

You can visit our [analytics site](https://info.brainaut.net/) to see a more advanced analytics page, or visit [the github](https://github.com/Brainaut/brainaut-info) for more detailed examples of using the Brainaut subgraph to create UIs.
...
function App() {
  const { loading: ethLoading, data: ethPriceData } = useQuery(BNB_PRICE_QUERY)
