...
Unsurprisingly, the SDK needs some notion of an BEP-20 token to be able to function. This immediately raises the question of _where data about tokens comes from_.
...
```typescript
import { ChainId, Token, Fetcher } from '@simocos/sdk'
...
Now that we've explored how to define a token, let's talk about pairs. To read more about what Brainaut pairs are, see <Link to='/docs/v1/smart-contracts/pair'>Pair</Link>.

As an example, let's try to represent the DAI-WBNB pair.

## Identifying Data

Each pair consists of two tokens (see previous section). Note that WBNB used by the router is <Link to='/docs/v1/SDK/other-exports/#weth'>exported by the SDK</Link>.
...
```typescript
import { ChainId, Token, WBNB, Pair, TokenAmount } from '@simocos/sdk'
...
```typescript
import { ChainId, Token, WBNB, Fetcher } from '@simocos/sdk'
